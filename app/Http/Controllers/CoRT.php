...
class CoRT extends Controller
{
    public function home()
    {
        ...
        $jwar = DB::SELECT("select COUNT(*) as jum from pengguna a, rukun_warga b where a.RW_ID = b.RW_ID AND a.DUSUN_ID = '$idd' and a.LEVEL = 'Warga' and a.HAPUS = 0");
        $jber = DB::SELECT("select COUNT(*) as jum from berita where RW_ID = '$irw' AND RT_ID = '$irt' AND HAPUS = 0 ");
        $katb = DB::SELECT("select*from berita a, kategori b where a.KAT_ID = b.KAT_ID and a.RW_ID = '$irw' group by a.KAT_ID");
            
        return view('/kart/home',['data'=>$data,'dsn'=>$dsn,'rw'=>$rw,'jwar'=>$jwar,'jber'=>$jber,'irw'=>$irw,'katb'=>$katb]);
    }

    public function updkart(Request $request,$id)
    {
        $na = $request->nama;
        $em = $request->email;
        $us = $request->user;
    ...
    {   
        ...
        $ni = Session::get('nik');
        $idb = berita::getId();
        $katber = DB::SELECT("select*from kategori where HAPUS = 0");
        $berita = DB::SELECT("select*from berita a, pengguna b, kategori c where a.PENG_ID = b.PENG_ID and a.KAT_ID = c.KAT_ID and a.HAPUS = 0 and a.RT_ID = '$irt'");
        $katb = DB::SELECT("select*from berita a, kategori b where a.KAT_ID = b.KAT_ID and a.RT_ID = '$irt' and a.RW_ID = '$irw' group by a.KAT_ID");
        return view('/kart/dt_berita',['data'=>$berita,'idb'=>$idb,'katber'=>$katber,'irw'=>$irw,'irt'=>$irt,'katb'=>$katb]);
    }
    ...
    public function addber(Request $request)
    {
       ...
       $data = new berita();
        ...
        return redirect('databeritart')->with('addpeng','.');
    }
    ...
    public function updber(Request $request,$id)
    {
        ...
        return redirect('databeritart')->with('updpeng','.');
    }
    ...
    public function berita($id)
    {
        ...
        return view('/kart/berita',['data'=>$data,'irw'=>$irw,'katb'=>$katb]);
    }
    ...
    public function diskusi($id)
    {
        ...
        return view('/kart/diskusi',['katb'=>$katb,'data'=>$data,'chat'=>$chat,'idk'=>$idk]);
    }
